...
def enumeration_ask(X, e, bn):
    ...
    # X: the query variable, a string
    # e: observed value for variable E, a dict
    # bn: a Bayes net
    for boo in bn.variable_values(X):
        e_x = extend(e, X, boo)
        Q[boo] = enumerate_all(bn.variables, e_x, bn)
    return Q.normalize()
    # END_YOUR_CODE
...
def enumerate_all(variables, e, bn):
    ...
    # e is a dictionary
    # cpt is the conditional probability, e.g. (True, False): 0.001
    # .p() is a probability
    if Y in e:
        y = e[Y]
        res = Ynode.p(y, e) * enumerate_all(rest, e, bn) # Y is in e, so is True
        return res
    sum_ = 0
    for boo in bn.variable_values(Y):
        e_y = extend(e, Y, boo)
        sum_ += Ynode.p(boo, e) * enumerate_all(rest, e_y, bn)
    return sum_
    # END_YOUR_CODE
